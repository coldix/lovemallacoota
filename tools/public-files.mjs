import { existsSync, readdirSync } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

/** Crossword PDFs in pdf/ are the ones we have chosen to publish. Solutions stay in data/crossword/ until the next issue. */
function publishedCrosswordPdfs() {
  const dir = path.join(rootDir, "pdf");
  if (!existsSync(dir)) return [];
  return readdirSync(dir)
    .filter((file) => /^crossword-.*\.pdf$/.test(file))
    .map((file) => `pdf/${file}`);
}

// Files copied verbatim into the build. Pages are generated by Astro and are
// not listed here.
export const publicFiles = [
  "data/listings_accom.json",
  "data/listings_do.json",
  "data/listings_food.json",
  "data/listings_community.json",
  "data/listings_services.json",
  "data/archive-index.json",
  "data/site-version.json",
  "favicon.ico",
  "images/background-01.webp",
  "images/background-02.webp",
  "images/background-03.webp",
  "images/background-04.webp",
  "images/background-05.webp",
  "images/background-06.webp",
  "images/logo145x100.webp",
  // No page points at this any more — every page has a card of its own under
  // images/og. It keeps shipping because links shared before that change named
  // it, and a scraper that refetches one should not find a 404 where the
  // picture was.
  "images/og-hero.jpg",
  "images/oze-logo216x100.webp",
  "images/ColinA.webp",
  "audio/frank-stokes-diver.mp3",
  "audio/lawson-brady-camp.mp3",
  "audio/the-bar-lawson.mp3",
  "audio/farewell-to-barbara.mp3",
  "pdf/Love-of-Mallacoota.pdf",
  ...publishedCrosswordPdfs(),
  "robots.txt",
  "site.webmanifest",
];

// Cover photographs and share cards are per-week and per-page, so these
// directories ship whole rather than being listed file by file. images/og holds
// the 1200 x 630 Open Graph cards built by tools/build-og.mjs.
export const publicDirectories = [
  "assets",
  "images/covers",
  "images/articles",
  "images/bank",
  "images/listings",
  "images/editions",
  "images/bus",
  "images/og",
  "data/directory",
];
